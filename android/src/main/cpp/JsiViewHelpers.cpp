    ...
    auto scrollToChild = jsi::Function::createFromHostFunction(
            *runtime_,
            ...
            [=](jsi::Runtime &runtime,
                ...
                __android_log_print(ANDROID_LOG_VERBOSE,"😇", "scrollToChild");

                jni::local_ref<jstring> scrollNativeID = nullptr;
                jni::local_ref<jstring> childNativeID;
                double scrollViewId = -1;

                std::string rawChildNativeID = params
                        .getProperty(runtime, "childNativeID")
                        .asString(runtime)
                        .utf8(runtime);
                childNativeID = jni::make_jstring(rawChildNativeID);

                __android_log_print(ANDROID_LOG_VERBOSE,"😇", "rawChildNativeID");

                auto rawScrollNativeID = params
                        .getProperty(runtime, "scrollNativeID");
                if (rawScrollNativeID.isString()) {
                    scrollNativeID = jni::make_jstring(rawScrollNativeID.asString(runtime).utf8(runtime));
                    __android_log_print(ANDROID_LOG_VERBOSE,"😇", "rawChildNativeID: %s", rawScrollNativeID.asString(runtime).utf8(runtime).c_str());
                }

                auto rawScrollViewId = params.getProperty(runtime, "scrollViewId");
                if (rawScrollViewId.isNumber()) {
                    scrollViewId = params
                            .getProperty(runtime, "scrollViewId")
                            .asNumber();
                }

                double offset = params
                        .getProperty(runtime, "offset")
                        .asNumber();

                bool scrollToEnd = params
                        .getProperty(runtime, "scrollToEnd")
                        .asBool();

                auto method = javaPart_->getClass()->getMethod<void(jni::local_ref<JString>, jdouble, jni::local_ref<JString>, jdouble, jboolean)>("scrollToChild");

                method(javaPart_.get(), scrollNativeID, scrollViewId, childNativeID, offset, scrollToEnd);
